-- SBC Completion Overview - FINAL CORRECTED VERSION
-- Removed non-existent columns and used only existing ones
...
FROM sbc_challenges sc
LEFT JOIN sbc_user_progress sup ON sc.id = sup.challenge_id AND sup.is_completed = true
GROUP BY sc.id, sc.name, sc.is_active, sc.is_repeatable, sc.rewards_tickets, sc.wbc_card_reward
