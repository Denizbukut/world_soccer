-- SBC Completion Overview - CORRECTED VERSION
...
SELECT 
    ...
    sc.name as challenge_name,  -- Changed from sc.title to sc.name
...
GROUP BY sc.id, sc.name, sc.description, sc.is_active, sc.is_repeatable
ORDER BY sc.id;
...
FROM sbc_challenges sc
LEFT JOIN sbc_user_progress sup ON sc.id = sup.challenge_id AND sup.is_completed = true
GROUP BY sc.id, sc.name, sc.is_active, sc.is_repeatable, sc.rewards_tickets, sc.rewards_exp, sc.wbc_card_reward
...
GROUP BY c.id, c.name, c.rarity, sc.name
